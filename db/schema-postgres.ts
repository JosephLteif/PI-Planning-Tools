...
export const planningRounds = pgTable('planning_rounds', {
  roomId: text('room_id').notNull().references(() => rooms.id, { onDelete: 'cascade' }),
  storyKey: text('story_key').notNull(),
...
export const planningRoundParticipants = pgTable('planning_round_participants', {
  roomId: text('room_id').notNull().references(() => rooms.id, { onDelete: 'cascade' }),
  storyKey: text('story_key').notNull(),
  roundNumber: integer('round_number').notNull(),
  accountId: text('account_id').notNull().references(() => accounts.id, { onDelete: 'cascade' }),
...
export const roomVotingMembers = pgTable('room_voting_members', {
  roomId: text('room_id').notNull().references(() => rooms.id, { onDelete: 'cascade' }),
  accountId: text('account_id').notNull().references(() => accounts.id, { onDelete: 'cascade' }),
  joinedAt: text('joined_at').notNull(),
}, (table) => ({
