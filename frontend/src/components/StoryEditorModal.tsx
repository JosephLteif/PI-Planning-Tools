...
export function StoryEditorModal({ state, story, initialType = 'Feature', saving, onClose, onSave }: StoryEditorModalProps) {
  ...
  function updateType(type: string) {
    setDraft((current) => ({ ...current, type, epicId: type === 'Epic' ? null : current.epicId, serviceLinks: type === 'Epic' ? current.serviceLinks : current.epicId ? [] : current.serviceLinks }));
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const ai = draft.type === 'Epic' ? null : parseEstimate(draft.aiText);
  ...
  return (
    <div className="modal-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="modal modal-wide story-editor-modal" role="dialog" aria-modal="true" aria-labelledby="story-editor-title">
        <div className="modal-header"><div><p className="section-kicker">{story ? 'Edit story' : 'Story queue'}</p><h2 id="story-editor-title">{story ? 'Edit story' : 'Add a story'}</h2><p>{story ? 'Update the story details, epic link, and service assignment.' : 'Capture the story, choose an epic, and assign the delivering services.'}</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close">×</button></div>
