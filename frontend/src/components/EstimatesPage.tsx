...
function StoryQueue({ stories, selectedId, canManage, aiEnabled, sessionActive, sessionPaused, onSelect, onNew, onImport, onEdit, onMove, onDelete, onRevote }: StoryQueueProps) {
  ...
  return <section className="card queue-card">
     ...
     <div className="queue-footer"><span className="icon"><AppIcon name={sessionPaused ? 'pause' : 'clock'} size={14} /></span>{sessionActive ? 'Pause the session to edit, reorder, or add stories.' : sessionPaused ? 'Session paused. Add or edit stories, then resume when the queue is ready.' : 'Select a story to estimate, edit, reorder, or import more work.'}</div>
  </section>;
}
...
export function EstimatesPage({ room, state, user, saving, onSave, onVote, onJoin, onClearVotes, onRemoveVoter }: EstimatesPageProps) {
  ...
  const nextEpicOptions = state.stories
    .filter((story) => story.type === 'Epic' && story.id !== currentEpic?.id)
  ...
  async function nextStory() {
    if (nextStoryInEpic) await startRoundForStory(nextStoryInEpic.id);
  }

  async function nextEpic(epicId: string) {
    const option = nextEpicOptions.find((candidate) => candidate.epic.id === epicId);
    if (option) await startRoundForStory(option.story.id);
  }

  function selectStory(id: string) {
    if (round.phase !== 'idle' && round.phase !== 'paused' && id !== selectedId) return;
    setSelectedId(id);
    if (isObserver) return;
    const next = cloneState(state);
     ...
     <div className="workspace-grid">
       <section className="card estimator-card">
         ...
         {round.phase === 'idle' ? <div className="session-gate"><div className="session-gate-icon"><AppIcon name="clock" size={22} /></div><div><p className="section-kicker">Estimation session</p><h3>Start a session to estimate this story</h3><p>{canManage ? 'Choose the story context, then open a live round so members can join and vote together.' : 'The room owner will start an estimation session before voting opens.'}</p></div>{canManage ? <button className="primary-button" type="button" disabled={saving} onClick={() => void startRound()}>Start estimation session</button> : null}</div> : isPaused ? <div className="session-gate session-paused"><div className="session-gate-icon"><AppIcon name="pause" size={22} /></div><div><p className="section-kicker">Estimation session</p><h3>Session paused</h3><p>{canManage ? `Queue editing is unlocked. Resume when you are ready to continue ${state.stories.find((story) => story.id === round.storyId)?.title || 'the current story'}.` : 'The room owner paused this session. You can review the queue while the round is paused.'}</p></div>{canManage ? <button className="primary-button" type="button" disabled={saving} onClick={() => void resumeSession()}><AppIcon name="play" size={14} /> Resume session</button> : null}</div> : <>
           ...
           <div className="estimate-section"><div className="estimate-section-heading"><div><h3>{isVoting ? isObserver ? 'Watch the live round' : 'Choose your cards' : 'Round result'}</h3><p>{isVoting ? isObserver ? 'Observers can watch the round but cannot submit votes.' : currentPlayer?.joined ? 'Submit one or both estimates when you are ready.' : 'Join this round before choosing cards.' : `${round.submittedCount} submitted · cards are flipped`}</p></div><div className="estimate-heading-tools"><span className={`round-badge${round.cardFlipped ? ' is-flipped' : ''}`}>{round.cardFlipped ? 'Cards flipped' : round.mode === 'open' ? 'Cards open' : 'Cards hidden'} · Round {round.roundNumber} · {formatDuration(round.timerStartedAt, now)}</span>{isVoting && canManage ? <select className="round-mode-select" value={state.roomSettings.voteMode} onChange={(event) => void setVoteMode(event.target.value as 'hidden' | 'open')}><option value="hidden">Hidden votes</option><option value="open">Open votes</option></select> : null}</div></div>
           ...
           <div className={`pl-vote-summary${round.cardFlipped ? ' cards-flipped' : ''}`}>{round.players.map((player) => {
             ...
             const aiEstimate = observer ? '—' : displayVote(player.ai, player.aiSubmitted, isRevealed);
