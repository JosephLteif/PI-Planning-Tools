...
type StoryNavigationOption = {
  epic: Pick<Story, 'id' | 'title'>;
  story: Story;
};
...
function StoryQueue({ stories, selectedId, canManage, aiEnabled, sessionActive, storySelectionDisabled, sessionPaused, onSelect, onNew, onImport, onEdit, onMove, onDelete, onRevote }: StoryQueueProps) {
  ...
  const epicOptions = [...epics, ...(unassignedStories.length ? [{ id: 'unassigned', title: 'Unassigned stories' }] : [])];
  ...
  const selectedStoryGroupId = selectedStory?.type !== 'Epic'
    ? selectedStoryEpicId && epics.some((epic) => epic.id === selectedStoryEpicId)
      ? selectedStoryEpicId
      : unassignedStories.some((story) => story.id === selectedStory?.id) ? 'unassigned' : null
    : null;
  const [focusedEpicId, setFocusedEpicId] = useState(() => selectedStoryGroupId || epicOptions[0]?.id || '');
  const [queueCollapsed, setQueueCollapsed] = useState(true);

  useEffect(() => {
    const fallbackGroupId = selectedStoryGroupId || epicOptions[0]?.id || '';
    if (selectedStoryGroupId && focusedEpicId !== selectedStoryGroupId) setFocusedEpicId(selectedStoryGroupId);
    else if (!focusedEpicId || !epicOptions.some((epic) => epic.id === focusedEpicId)) setFocusedEpicId(fallbackGroupId);
  }, [epicOptions, focusedEpicId, selectedStoryGroupId]);

  const focusedGroup = groups.find((group) => group.key === focusedEpicId);
  ...
  function selectEpic(epicId: string) {
    ...
    const group = groups.find((candidate) => candidate.key === epicId);
  ...
  const visibleGroups = focusedEpicId ? groups.filter((group) => group.key === focusedEpicId) : groups;
  ...
  return <section className={`card queue-card${queueCollapsed ? ' queue-card-collapsed' : ''}`}>
     <div className="queue-header"><div><h2>Story queue</h2><p>{estimableCount} {estimableCount === 1 ? 'story' : 'stories'} · {estimatedCount} estimated</p><p className="queue-commitment">Committed to <strong>{formatStoryPoints(commitment.committed)} SPs</strong> / <strong>{formatStoryPoints(commitment.stretch)} SPs</strong> stretch</p></div><div className="queue-header-actions">{canManage ? <><button className="outline-button import-button" type="button" disabled={sessionActive} onClick={() => onNew()}><AppIcon name="plus" size={13} /> Story</button><button className="outline-button import-button" type="button" disabled={sessionActive} onClick={() => onNew('Epic')}><AppIcon name="plus" size={13} /> Epic</button><button className="outline-button import-button" type="button" disabled={sessionActive} onClick={onImport}><AppIcon name="upload" size={13} /> Import</button></> : null}<button className="queue-toggle" type="button" aria-expanded={!queueCollapsed} aria-controls="story-queue-content" onClick={() => setQueueCollapsed((current) => !current)}><AppIcon name={queueCollapsed ? 'chevronDown' : 'chevronUp'} size={14} /> {queueCollapsed ? 'Show queue' : 'Hide queue'}</button></div></div>
     {!queueCollapsed ? <div id="story-queue-content"><>{focusedGroup ? <div className="epic-selector"><span className={`epic-selector-mark${focusedTotal > 0 && focusedDone === focusedTotal ? ' is-complete' : ''}`} aria-hidden="true"><AppIcon name={focusedTotal > 0 && focusedDone === focusedTotal ? 'check' : 'clock'} size={15} /></span><div className="epic-selector-copy"><span>{focusedEpic ? `Epic needing work${focusedEpic.stretch ? ' · Stretch' : ''}` : 'Unassigned stories'}</span><div className="epic-selector-select-row"><EpicPicker epics={epicOptions} value={focusedEpicId} onChange={selectEpic} ariaLabel="Story group" disabled={storySelectionDisabled} optionMeta={(epic) => { const children = groups.find((group) => group.key === epic.id)?.children || []; return `${children.filter((story) => story.manual !== null).length}/${children.length} done`; }} />{canManage && focusedEpic ? <div className="epic-selector-actions"><button className="story-action-button" type="button" disabled={sessionActive} onClick={() => onEdit(focusedEpic)} aria-label={`Edit ${focusedEpic.title}`}><AppIcon name="pencil" size={13} /></button><button className="story-action-button danger-action" type="button" disabled={sessionActive || stories.length <= 1} onClick={() => onDelete(focusedEpic)} aria-label={`Delete ${focusedEpic.title}`}><AppIcon name="trash" size={13} /></button></div> : null}</div></div><span className="epic-selector-progress">{focusedDone}/{focusedTotal}</span><span className="epic-selector-status">{focusedTotal > 0 && focusedDone === focusedTotal ? 'Done' : focusedTotal ? 'In progress' : 'No stories'}</span></div> : null}</><div className="story-queues">{visibleGroups.map((group) => group.epic ? <div className="story-queue-group" key={group.key}><div className="story-list">{group.children.length ? group.children.map(storyRow) : <p className="empty-manager">No stories linked to this epic yet.</p>}</div></div> : <div className="story-list" key={group.key}>{group.children.map(storyRow)}</div>)}</div><div className="queue-footer"><span className="icon"><AppIcon name={sessionPaused ? 'pause' : 'clock'} size={14} /></span>{sessionActive ? canManage ? 'Select any story to switch the active vote.' : 'The room manager controls the active story.' : sessionPaused ? 'Session paused. Add or edit stories, then resume when the queue is ready.' : 'Select a story to estimate, edit, reorder, or import more work.'}</div></div> : <div className="queue-collapsed-note"><AppIcon name="listChecks" size={15} /><span>Queue hidden. Use the story picker above to change the current story.</span></div>}
  </section>;
}
...
export function EstimatesPage({ room, state, user, focusMode = false, saving, onSave, onVote, onJoin, onClearVotes, onRemoveVoter }: EstimatesPageProps) {
  ...
  const currentEpic = currentStory.epicId ? state.stories.find((story) => story.id === currentStory.epicId && story.type === 'Epic') || null : null;
  const actualEpics = state.stories.filter((story) => story.type === 'Epic');
  const unassignedStories = estimableStories.filter((story) => !story.epicId || !actualEpics.some((epic) => epic.id === story.epicId));
  const currentStoryIsUnassigned = unassignedStories.some((story) => story.id === currentStory.id);
  const currentEpicStories = currentEpic ? state.stories.filter((story) => story.type !== 'Epic' && story.epicId === currentEpic.id) : [];
  const currentEpicIndex = currentEpicStories.findIndex((story) => story.id === currentStory.id);
  const nextStoryInEpic = currentEpicIndex >= 0 ? currentEpicStories[currentEpicIndex + 1] || null : estimableStories[estimableStories.findIndex((story) => story.id === currentStory.id) + 1] || null;
  const nextUnassignedStory = unassignedStories.find((story) => story.id !== currentStory.id && story.manual === null) || unassignedStories.find((story) => story.id !== currentStory.id) || null;
  const actualNextEpicOptions = actualEpics
    .filter((epic) => epic.id !== currentEpic?.id)
    .map((epic) => ({ epic, story: state.stories.find((story) => story.type !== 'Epic' && story.epicId === epic.id && story.manual === null) || state.stories.find((story) => story.type !== 'Epic' && story.epicId === epic.id) || null }))
    .filter((option): option is { epic: Story; story: Story } => option.story !== null);
  const nextEpicOptions: StoryNavigationOption[] = nextUnassignedStory && (!currentStoryIsUnassigned || unassignedStories.length > 1)
    ? [...actualNextEpicOptions, { epic: { id: 'unassigned', title: 'Unassigned stories' }, story: nextUnassignedStory }]
    : actualNextEpicOptions;
  ...
  async function skipStory() {
    const nextStory = nextStoryInEpic || nextEpicOptions[0]?.story || estimableStories.find((story) => story.id !== currentStory.id);
    if (!nextStory) return;
    const next = cloneState(state);
    next.selectedStoryId = nextStory.id;
    next.round = { ...next.round, phase: 'idle', storyId: nextStory.id, roundNumber: 1, votes: {}, submittedCount: 0, cardFlipped: false, revealedAt: null, timerStartedAt: null, timerEndsAt: null };
    setSelectedId(nextStory.id);
    await onSave(next);
  }

  async function nextStory() {
    if (nextStoryInEpic) await startRoundForStory(nextStoryInEpic.id, nextRoundNumberForStory(nextStoryInEpic.id));
  }

  async function nextEpic(epicId: string) {
    const option = nextEpicOptions.find((candidate) => candidate.epic.id === epicId);
    if (option) await startRoundForStory(option.story.id, nextRoundNumberForStory(option.story.id));
  }
  ...
  return <>
    ...
    {!focusMode ? <div className="summary-grid"><div className="summary-card"><div className="summary-top"><span className="summary-label">Stories</span><span className="summary-icon"><AppIcon name="listChecks" size={16} /></span></div><div className="summary-value">{estimableStories.length}</div><div className="summary-foot">Ready for the next discussion</div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Estimated</span><span className="summary-icon"><AppIcon name="check" size={16} /></span></div><div className="summary-value">{estimableStories.filter((story) => story.manual !== null).length}<small>/ {estimableStories.length}</small></div><div className="progress-bar"><span style={{ width: `${estimableStories.length ? (estimableStories.filter((story) => story.manual !== null).length / estimableStories.length) * 100 : 0}%` }} /></div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Planners</span><span className="summary-icon"><AppIcon name="users" size={16} /></span></div><div className="summary-value">{room.memberCount}</div><div className="summary-foot">People in this room</div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Round</span><span className="summary-icon"><AppIcon name={isPaused ? 'pause' : 'clock'} size={16} /></span></div><div className="summary-value">{round.phase === 'voting' ? 'Live' : round.phase === 'revealed' ? 'Done' : isPaused ? 'Paused' : 'Idle'}</div><div className="summary-foot">{round.phase === 'voting' ? (countVisible ? `${round.submittedCount} vote${round.submittedCount === 1 ? '' : 's'} submitted` : 'Submitted values appear as voters submit') : isPaused ? 'Queue editing is unlocked' : progressLabel}</div></div></div> : null}
     <div className={`workspace-grid${focusMode ? ' voting-page-grid' : ''}`}>
       <section className="card estimator-card">
         <div className="card-heading"><div><span className="story-progress">{selectedStory.id} · {selectedStory.type}</span><h2>{focusMode ? 'Vote on this story' : 'Estimate this story'}</h2></div><div className="card-heading-controls"><label className="sequence-control story-switch-control"><span>{focusMode ? 'Choose story' : sessionActive ? 'Switch story' : 'Story'}</span><select className="story-switch-select" value={currentStory.id} disabled={saving || (sessionActive && !canManage)} aria-label="Story to vote on" onChange={(event) => selectStory(event.target.value)}>{estimableStories.map((story) => <option value={story.id} key={story.id}>{story.id} · {story.title}</option>)}</select></label><label className="sequence-control">Sequence<select value={state.sequence} disabled={!canManage || saving} onChange={(event) => { const next = cloneState(state); next.sequence = event.target.value as RoomState['sequence']; void onSave(next); }}><option value="sequential">Sequential</option><option value="fibonacci">Fibonacci</option><option value="modified">Modified Fibonacci</option></select></label></div></div>
         {round.phase === 'idle' ? <div className="session-gate"><div className="session-gate-icon"><AppIcon name="clock" size={22} /></div><div><p className="section-kicker">Estimation session</p><h3>Start a session to estimate this story</h3><p>{canManage ? 'Choose the story context, then open a live round so members can join and vote together.' : 'The room owner will start an estimation session before voting opens.'}</p></div>{canManage ? <button className="primary-button" type="button" disabled={saving} onClick={() => void startRound()}>Start estimation session</button> : null}</div> : isPaused ? <div className="session-gate session-paused"><div className="session-gate-icon"><AppIcon name="pause" size={22} /></div><div><p className="section-kicker">Estimation session</p><h3>Session paused</h3><p>{canManage ? `Queue editing is unlocked. Resume when you are ready to continue ${state.stories.find((story) => story.id === round.storyId)?.title || 'the current story'}.` : 'The room owner paused this session. You can review the queue while the round is paused.'}</p></div>{canManage ? <button className="primary-button" type="button" disabled={saving} onClick={() => void resumeSession()}><AppIcon name="play" size={14} /> Resume session</button> : null}</div> : <>
           ...
           <div className="estimator-footer"><div className="status-message"><span className="icon"><AppIcon name="clock" size={14} /></span>{isVoting ? isObserver ? 'Observer · view only' : currentPlayer?.joined ? (countVisible ? `${round.submittedCount} vote${round.submittedCount === 1 ? '' : 's'} submitted` : 'Submitted values appear as voters submit') : 'You have not joined this round' : selectedStory.manual === null ? 'No final estimate saved' : `Final estimate: ${selectedStory.manual}`}</div><div className="footer-actions">{isVoting && !isObserver && !currentPlayer?.joined ? <button className="outline-button" type="button" disabled={saving} onClick={() => void onJoin(true)}>Join round</button> : null}{isVoting && !isObserver && currentPlayer?.joined ? <><button className="outline-button" type="button" disabled={saving} onClick={() => void onJoin(false)}>Leave round</button><button className="primary-button" type="button" disabled={saving || (manual === null && (!aiEnabled || ai === null))} onClick={() => void onVote(manual, aiEnabled ? ai : null)}>Submit vote</button></> : null}{isVoting && canManage ? <><button className="outline-button" type="button" disabled={saving} onClick={() => void revealRound()}>Flip cards</button><button className="outline-button" type="button" disabled={saving} onClick={() => void onClearVotes()}>Clear votes</button><button className="outline-button" type="button" disabled={saving} onClick={() => void resetTimer()}>Reset timer</button><button className="outline-button" type="button" disabled={saving} onClick={() => void skipStory()}>Skip story</button><button className="outline-button" type="button" disabled={saving} onClick={() => void pauseSession()}><AppIcon name="pause" size={13} /> Pause session</button></> : null}{isRevealed && canManage ? <>{nextStoryInEpic ? <button className="outline-button" type="button" disabled={saving} onClick={() => void nextStory()}>Next story</button> : null}{estimableStories.some((story) => story.id !== currentStory.id) ? <select className="round-next-story-select" defaultValue="" disabled={saving} aria-label="Choose a specific story" onChange={(event) => { if (event.target.value) void startRoundForStory(event.target.value, nextRoundNumberForStory(event.target.value)); }}><option value="">Choose any story…</option>{estimableStories.filter((story) => story.id !== currentStory.id).map((story) => <option value={story.id} key={story.id}>{story.id} · {story.title}</option>)}</select> : null}{nextEpicOptions.length ? <select className="round-next-epic-select" defaultValue="" disabled={saving} aria-label="Choose another epic" onChange={(event) => { if (event.target.value) void nextEpic(event.target.value); }}><option value="">Next epic…</option>{nextEpicOptions.map((option) => <option value={option.epic.id} key={option.epic.id}>{option.epic.title}</option>)}</select> : <span className="round-next-copy">Last story in this epic</span>}<button className="outline-button" type="button" disabled={saving} onClick={() => void resetRound()}>Restart story</button><button className="primary-button" type="button" disabled={saving} onClick={() => void saveEstimate()}>Save final estimate</button><button className="outline-button" type="button" disabled={saving} onClick={() => void pauseSession()}><AppIcon name="pause" size={13} /> Pause session</button></> : null}</div></div>
