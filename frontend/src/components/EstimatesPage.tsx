...
function StoryQueue({ stories, selectedId, canManage, aiEnabled, sessionActive, onSelect, onNew, onImport, onEdit, onMove, onDelete, onRevote }: StoryQueueProps) {
  ...
  return <section className="card queue-card">
     ...
     <div className="queue-footer"><span className="icon"><AppIcon name="clock" size={14} /></span>{sessionActive ? 'Queue actions are paused while the estimation session is live.' : 'Select a story to estimate, edit, reorder, or import more work.'}</div>
  </section>;
}
...
export function EstimatesPage({ room, state, user, saving, onSave, onVote, onJoin, onClearVotes, onRemoveVoter }: EstimatesPageProps) {
  ...
  function selectStory(id: string) {
    if (round.phase !== 'idle' && id !== selectedId) return;
    setSelectedId(id);
    if (isObserver) return;
    const next = cloneState(state);
