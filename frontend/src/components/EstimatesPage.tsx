...
function StoryQueue({ stories, selectedId, canManage, aiEnabled, sessionActive, storySelectionDisabled, sessionPaused, onSelect, onNew, onImport, onEdit, onMove, onDelete, onRevote }: StoryQueueProps) {
  ...
  return <section className="card queue-card">
     ...
     <div className="queue-footer"><span className="icon"><AppIcon name={sessionPaused ? 'pause' : 'clock'} size={14} /></span>{sessionActive ? canManage ? 'Select any story to switch the active vote.' : 'The room manager controls the active story.' : sessionPaused ? 'Session paused. Add or edit stories, then resume when the queue is ready.' : 'Select a story to estimate, edit, reorder, or import more work.'}</div>
  </section>;
}
...
export function EstimatesPage({ room, state, user, saving, onSave, onVote, onJoin, onClearVotes, onRemoveVoter }: EstimatesPageProps) {
  ...
  return <>
    ...
    <div className="summary-grid"><div className="summary-card"><div className="summary-top"><span className="summary-label">Stories</span><span className="summary-icon"><AppIcon name="listChecks" size={16} /></span></div><div className="summary-value">{estimableStories.length}</div><div className="summary-foot">Ready for the next discussion</div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Estimated</span><span className="summary-icon"><AppIcon name="check" size={16} /></span></div><div className="summary-value">{estimableStories.filter((story) => story.manual !== null).length}<small>/ {estimableStories.length}</small></div><div className="progress-bar"><span style={{ width: `${estimableStories.length ? (estimableStories.filter((story) => story.manual !== null).length / estimableStories.length) * 100 : 0}%` }} /></div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Planners</span><span className="summary-icon"><AppIcon name="users" size={16} /></span></div><div className="summary-value">{room.memberCount}</div><div className="summary-foot">People in this room</div></div><div className="summary-card"><div className="summary-top"><span className="summary-label">Round</span><span className="summary-icon"><AppIcon name={isPaused ? 'pause' : 'clock'} size={16} /></span></div><div className="summary-value">{round.phase === 'voting' ? 'Live' : round.phase === 'revealed' ? 'Done' : isPaused ? 'Paused' : 'Idle'}</div><div className="summary-foot">{round.phase === 'voting' ? (countVisible ? `${round.submittedCount} vote${round.submittedCount === 1 ? '' : 's'} submitted` : 'Submitted values appear as voters submit') : isPaused ? 'Queue editing is unlocked' : progressLabel}</div></div></div>
     ...
     {state.voteHistory.length ? <section className="card history-card" style={{ marginTop: 18 }}><div className="lower-card-heading"><div><h2>Vote history</h2><p>Revealed rounds saved for this planning room.</p></div><span>{state.voteHistory.length} votes</span></div><div className="table-scroll"><table className="history-table"><thead><tr><th>Story</th><th>Round</th><th>Planner</th><th>Manual</th>{aiEnabled ? <th>AI</th> : null}</tr></thead><tbody>{[...state.voteHistory].sort((left, right) => right.updatedAt?.localeCompare(left.updatedAt || '') || 0).slice(0, 30).map((entry) => <tr key={`${entry.storyId}-${entry.roundNumber}-${entry.voterId}`}><td>{state.stories.find((story) => story.id === entry.storyId)?.title || entry.storyId}</td><td>#{entry.roundNumber}</td><td>{entry.voterId === user.id ? 'You' : entry.voterName}</td><td className="table-score">{entry.manual ?? '—'}</td>{aiEnabled ? <td className="table-score">{entry.ai ?? '—'}</td> : null}</tr>)}</tbody></table></div></section> : null}
