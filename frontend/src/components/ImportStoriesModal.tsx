...
type ImportStoriesModalProps = {
  saving: boolean;
  onClose: () => void;
  onImport: (stories: Story[]) => Promise<void>;
};

export function ImportStoriesModal({ saving, onClose, onImport }: ImportStoriesModalProps) {
  ...
  return (
    <div className="modal-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="import-stories-title">
        ...
        <div className="modal-footer"><button className="outline-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="button" disabled={saving || !stories.length} onClick={() => void onImport(stories)}>Import stories ↑</button></div>
      </section>
    </div>
  );
}
